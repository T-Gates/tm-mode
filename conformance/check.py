...
# handlers/*.py 전용 토큰 리터럴 탐지 — 값 자체의 접두사로 판단 (S1-D).
# 일반 소스는 키-이름 기반이지만, 핸들러는 credentials.load() 를 강제하므로
# 값이 Bearer·sk-·xoxb- 등으로 시작하는 문자열 리터럴 자체가 위반.
#
# 확장 (codex fault injection 반영):
#   - sk-proj- 등 하이픈 포함 패턴: sk-[A-Za-z0-9_-]{10,}
#   - GitHub 토큰: gh[pousr]_, github_pat_
#   - AWS: AKIA[A-Z0-9]{16}, ASIA[A-Z0-9]{16}
#   - Basic <base64>: HTTP Basic Authorization 헤더값
#   - 일반 40자 hex: git commit hash 같은 토큰 형식
...
# 문자열 분할 우회·base64 인코딩 우회 탐지용 보조 패턴
# "xox" + "b-..." 같은 concatenation 이나 bytes literal 탐지는
# tokenize 기반 스캔(_handler_secret_hit_source)에서 처리한다.
...
# 예: "xox" + "b-1234-..." 를 라인 단위로 탐지하는 대신
# tokenize 로 STRING 토큰을 모아 인접 concatenation 감지
...
def _decode_base64_token(s: str) -> str | None:
...
def _source_has_handler_token(source: str) -> bool:
    """핸들러 소스 전체를 tokenize 기반으로 스캔.

    탐지 대상:
    1. STRING/COMMENT 토큰에서 직접 토큰 패턴 매칭
    2. 인접 STRING 상수 연결(constant folding 우회) — 합산 후 재검사
    3. bytes literal 안 토큰 패턴
    4. base64 인코딩 우회 — base64 후보 디코딩 후 재검사
    5. f-string 분할 우회 — FSTRING 내부 STRING 부분 합산

    placeholder 값을 포함하면 허용 (기존 규칙 유지).
    """
    try:
        tokens = list(tokenize.generate_tokens(io.StringIO(source).readline))
    except (tokenize.TokenError, IndentationError, SyntaxError):
        # 불완전·문법오류 소스도 최대한 검사 — 라인 기반 폴백 (malformed = 차단이 안전)
        return _handler_secret_hit_multiline(source)

    # STRING 토큰 값을 모아 인접 concatenation 감지
    string_values: list[str] = []
    prev_was_string = False

    # Python 3.12+ 에서 f-string 은 FSTRING_START/MIDDLE/END 로 분리된다.
    # FSTRING_MIDDLE 토큰은 f-string 의 리터럴 부분 — 변수 표현식 제외한 텍스트.
    # 예: f"sk-proj-{part}" → FSTRING_MIDDLE="sk-proj-"
    ...
    for tok_type, tok_string, tok_start, tok_end, tok_line in tokens:
        ...
        # Python 3.12+ FSTRING_MIDDLE: f-string 의 리터럴 부분 직접 검사
        if _FSTRING_MIDDLE is not None and tok_type == _FSTRING_MIDDLE:
            # 리터럴 부분에 토큰 접두사가 있으면 탐지 (f"sk-proj-{part}" 등)
        ...
        if tok_type == tokenize.STRING:
            # 문자열 리터럴 파싱 — bytes·f-string·raw·삼중따옴표 모두 처리
            raw = tok_string
            # bytes literal: b"..." / B'...'
            if raw.startswith(("b'", 'b"', "B'", 'B"', "rb", "br", "RB", "BR")):
                ...
                prev_was_string = False
                continue
            # f-string: f"..." / F"..." / rf"..." / fr"..." 등 — ast.literal_eval 불가
            # raw 토큰 문자열 자체를 재검사 (f-string 분할 우회 차단).
            # _handler_secret_hit_line_raw 외에 _CONCAT_TOKEN_RE 로도 리터럴 부분 탐지
            # (예: f"sk-proj-{part}" 에서 'sk-proj' 접두가 리터럴로 포함돼 있음).
            ...
            if raw.startswith(_FSTRING_PREFIXES):
                if _handler_secret_hit_line_raw(raw):
                    return True
                # f-string 리터럴 부분에 토큰 접두사가 있으면 탐지
            ...
            inner = _extract_string_inner(raw)
            if inner is None:
                # literal_eval 실패 (비-f-string 파싱 불가) — raw 라인 재검사 후 skip
                if _handler_secret_hit_line_raw(raw):
                    return True
                prev_was_string = False
                continue

            # placeholder 체크: 전체 값이 placeholder 와 정확히 일치할 때만 skip
            # 부분 포함은 허용하지 않음 (AKIA...EXAMPLE 같은 실제 토큰에 'example' 포함 오탐 방지)
        ...
        # OP(+) 사이에 STRING이 있을 수 있으므로 + 연산자는 prev_was_string 유지
        if tok_type == tokenize.OP and tok_string == "+":
            # prev_was_string 그대로 유지 (string + string 연결)
            continue
...
def _extract_string_inner(raw: str) -> str | None:
...
def _raw_token_match(s: str) -> bool:
    """문자열 s에 토큰 패턴이 있는지 확인 (placeholder 제외).

    placeholder 체크: 전체 값이 placeholder 와 정확히 일치할 때만 skip.
    부분 포함(예: 'AKIAIOSFODNN7EXAMPLE'에 'example'이 들어있는 경우)은 허용하지 않는다.
    """
    s_lower = s.strip().lower()
    # 전체 값이 placeholder 와 정확히 일치하면 skip
    ...
    if _HANDLER_TOKEN_BYTES_RE.search(f'b"{s}"'):
        return True
    # CONCAT 패턴이 있고 길이가 토큰 길이 이상이면 의심
...
def _handler_secret_hit_line_raw(line: str) -> bool:
    """단일 라인에서 핸들러 토큰 패턴 탐지 (tokenize 미사용).

    placeholder 체크: 전체 값이 정확히 일치할 때만 skip (부분 포함 오탐 방지).
    """
    # bytes literal 탐지
    ...
    for m in _BASE64_CANDIDATE_RE.finditer(line):
        ...
        if decoded and _raw_token_match(decoded):
            return True
    return _secret_hit_line(line)


def _handler_secret_hit_line(line: str) -> bool:
    """handlers/*.py 라인에 토큰 리터럴 직접값이 있는가.

    placeholder 값(_SECRET_VALUE_ALLOW 공유)은 허용.
    credentials.load() 호출·변수명·주석 내부는 이 패턴에 걸리지 않는다.
    이 함수는 lint_no_tracked_secrets 의 라인 루프에서 사용 (기존 API 유지).
    소스 전체 스캔이 가능하면 _source_has_handler_token 을 우선 사용한다.
    """
    return _handler_secret_hit_line_raw(line)
...
def lint_no_tracked_secrets(root, *, files=None) -> tuple:
    ...
    if files is None:
        ...
        for p in scan:
            ...
            if p not in seen:
                seen.add(p)
                candidates.append(p)
        # handlers/**/*.py — suffix skip 없이 강제 스캔 (S1-D, 재귀 포함).
        # _SECRET_TARGET_SKIP_SUFFIXES 에 .py 가 있어 일반 소스는 제외되지만,
        # 핸들러 코드는 토큰이 직접 embed될 수 있는 "데이터처럼 취급"해야 한다.
        # skip suffix 목록을 건드리지 않고, handlers/ 를 명시 수집으로 강제 포함.
        # 하위 디렉토리(handlers/sub/x.py)도 재귀 스캔 (codex fault injection 반영).
        # .venv·venv·__pycache__·.git·site-packages 는 prune (codex fault injection #5).
        ...
        if handlers_dir.is_dir():
            for hp in sorted(handlers_dir.rglob("*.py")):
                if not hp.is_file():
                    continue
        ...
        # files= 경로에서도 root/handlers 하위 .py 를 handler 로 분류 (codex fault injection #2).
        # files 주입 경로가 handler lint 를 우회하지 않도록 동일 hit 함수 적용.
        handler_files = set()
        if root.is_dir():
            handlers_dir = root / "handlers"
            for p in candidates:
                try:
                    rel = Path(p).resolve().relative_to(handlers_dir.resolve())
                    handler_files.add(Path(p))
                except (ValueError, OSError):
                    pass
        else:
            # root 가 tmp 등 임시 경로일 때: 경로 문자열로 handlers/ 포함 여부 판정
            for p in candidates:
                pp = Path(p)
                parts = pp.parts
                if "handlers" in parts:
                    handler_files.add(pp)
    ...
    for p in candidates:
        ...
        try:
            ...
            rel = p
        # handlers/**/*.py 는 전용 tokenize 기반 소스 전체 스캔 우선 (S1-D, codex반영).
        # 라인 기반 hit_fn 은 폴백·비핸들러용으로 유지.
        if p in handler_files:
            if _source_has_handler_token(text):
                # 정확한 라인 번호 보고를 위해 라인별로도 체크
                line_hit = False
...
def lint_handlers_contract(root) -> tuple:
    """handlers/**/*.py 가 계약을 만족하는지 정적 검사 (S1, codex fault injection #4).

    검사 항목:
    1. 역할별 필수 함수가 **top-level** 에 정의돼 있는지 (ast.Module.body 직속)
       — nested function·class method·인자 없는 함수도 이름만 있으면 통과하던 버그 수정.
    2. 필수 함수 시그니처가 계약 표(docs/spec/handlers.md §2)와 일치하는지.
    3. async 함수(AsyncFunctionDef)는 **허용** — S7 이전에 async 핸들러를 쓸 수 있다.
       (이유: asyncio 기반 서비스 SDK 가 증가 추세. 동기·비동기 모두 계약 만족으로 인정)
    4. 핸들러 소스 전체를 tokenize 기반으로 secret 탐지 (분할·base64 우회 차단).

    반환: (검사명, 통과여부, 상세) — run_lint 표준 tuple 형.
    handlers/ 없으면 건너뜀 (S7 이전은 정상 — True).
    """
    root = Path(root)
    handlers_dir = root / "handlers"
    if not handlers_dir.is_dir():
        return ("handlers 계약 검사", True, "handlers/ 없음 — 건너뜀")

    # .venv·venv·__pycache__·.git·site-packages 는 prune (codex fault injection #5)
    _PRUNE_DIRS_CONTRACT = frozenset({".venv", "venv", "__pycache__", ".git", "site-packages"})
    py_files = [
        p for p in sorted(handlers_dir.rglob("*.py"))
        if not any(part in _PRUNE_DIRS_CONTRACT for part in p.parts)
    ]
    if not py_files:
        return ("handlers 계약 검사", True, "handlers/ 비어 있음 — 건너뜀")

    # 역할별 필수 함수 + 계약 시그니처 (docs/spec/handlers.md §2 표)
    # 형식: 함수명 → (필수 인자 목록, 선택 인자 목록, **kwargs 허용)
    _CONTRACT: dict[str, list[tuple[str, list[str], list[str], bool]]] = {
        "issues": [
            ("issues_create", ["title"], ["body", "assignee", "label", "priority"], True),
            ("issues_list",   [],        ["filter"],                                  True),
            ("issues_get",    ["id"],    [],                                          True),
            ("issues_update", ["id"],    [],                                          True),  # **kwargs
        ],
        "chat": [
            ("chat_send", ["message"], ["channel"],        True),
            ("chat_list", [],          ["channel", "limit"], True),
        ],
        "docs": [
            ("docs_read",   ["id"],             [],          True),
            ("docs_write",  ["id", "content"],  [],          True),
            ("docs_list",   [],                 ["query"],   True),
            ("docs_create", ["title"],          ["content"], True),
        ],
        "calendar": [
            ("calendar_list",   ["start"],         ["end"],                          True),
            ("calendar_create", ["title", "start"], ["end", "description"],          True),
        ],
    }
    ...
    for py_file in py_files:
        ...
        try:
            ...
            errors.append(f"{py_file.name}: 읽기 실패 ({e})")
            continue
        ...
        try:
            ...
            errors.append(f"{py_file.name}: 문법 오류 ({e})")
            continue

        # top-level 함수만 수집 (ast.Module.body 직속 FunctionDef·AsyncFunctionDef)
        # nested function·class method 은 인정하지 않는다 (codex fault injection #3).
        top_level_funcs: dict[str, ast.FunctionDef] = {}
        for node in tree.body:
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                top_level_funcs[node.name] = node

        # 역할 계약 검사 (role 이 계약 표에 있는 경우만)
        if role in _CONTRACT:
            for fn_name, required_args, optional_args, kwargs_allowed in _CONTRACT[role]:
                if fn_name not in top_level_funcs:
                    errors.append(
                        f"{py_file.name}: top-level 함수 '{fn_name}' 없음"
                        f" (nested/method 는 인정 안 됨)"
                    )
                    continue

                fn_node = top_level_funcs[fn_name]
                fn_args = fn_node.args

                # 실제 인자 정보 추출
                posargs = [a.arg for a in fn_args.args]
                vararg = fn_args.vararg  # *args
                kwonlyargs = [a.arg for a in fn_args.kwonlyargs]
                kwarg = fn_args.kwarg   # **kwargs
                defaults_count = len(fn_args.defaults)
                required_count = len(posargs) - defaults_count

                # 시그니처 검증 (codex fault injection #2):
                # *args + **kwargs 조합(완전 범용 시그니처)은 계약 호환으로 허용한다.
                # 단, *args 단독(vararg 있고 kwarg 없음)은 계약 호환으로 인정하지 않는다:
                #   - *args 는 keyword 인자를 받지 못하므로 완전한 계약 이행 불가.
                #   - required 인자 명시 검증을 우회하는 수단으로 쓰이는 것을 차단.
                # **kwargs 단독(kwarg 있고 vararg 없음)은 required positional 인자를
                #   대체하지 못한다 — required 인자는 명시적으로 posargs 에 있어야 함.
                # (기존 동작: *args + **kwargs 둘 다 있으면 범용 → 통과 유지)
                if vararg is not None and kwarg is not None:
                    # *args + **kwargs: 완전 범용 시그니처 — 계약 호환 허용
                    continue

                # 필수 인자 충족 여부 확인
                # posargs 에 required 인자명이 있어야 한다.
                # *args 단독이면 posargs 에 required 인자가 없으면 실패.
                # **kwargs 단독이면 posargs 에 required 인자가 없으면 실패.
                all_named_args = posargs + kwonlyargs
                for req in required_args:
                    if req not in all_named_args:
                        errors.append(
                            f"{py_file.name}: '{fn_name}' 필수 인자 '{req}' 없음"
                            f" (실제 positional={posargs}, kwonly={kwonlyargs},"
                            f" vararg={'*'+vararg.arg if vararg else None},"
                            f" kwarg={'**'+kwarg.arg if kwarg else None})"
                        )

        # secret tokenize 스캔 (분할·base64 우회 포함)
        if _source_has_handler_token(source):
            errors.append(f"{py_file.name}: 토큰 리터럴 직접 embed 탐지")

    return ("handlers 계약 검사", not errors,
            f"위반 {len(errors)}건: {errors}" if errors else "")
...
def run_lint(root) -> LintReport:
    ...
    checks.append(lint_handlers_contract(root))

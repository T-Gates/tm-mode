...
|---|---|
| spec_version | **0.1** |
| 상태 | 정식 단일판 (reconciled with build, 2026-06-15) |
...
2. **MCP 표기 규약(필수)**: 스킬 본문은 시맨틱 참조만 — "Linear MCP의 list_issues로 조회". `mcp__*` 직표기 금지(lint 대상). 에이전트별 실제 툴명 형식은 각 에이전트 진입 문서(CLAUDE.md/AGENTS.md류)에 1회만.
...
엔진 `infra/teammode.py`는 7개 동사를 구현한다. **공통 계약(필수)**: 팀 루트는 `--root`로만 받고(§1.2 P1), env 무신뢰, `--root` 미지정 시 exit 2. 알 수 없는 동사는 exit 127(`[unimplemented]`), 동사 없으면 usage + exit 2. 엔진은 **요약·판단을 하지 않는다**(기계적 재료손질만 — 요약은 스킬/에이전트 몫).

```
teammode.py <verb> --root <팀루트> [동사별 플래그]
verbs: on | off | log | context | pull | commit | update
```

값 받는 플래그 화이트리스트: `--root --settings --author --text --now --message`. 부울: `--install --json --push`. 화이트리스트 밖 `--flag`의 다음 토큰은 값으로 삼키지 않는다(verb 손실 방지).
...
- **on**: ① 배너 출력(`_render_banner`: `memory/banner.txt` 있으면 그대로, 없으면 `TGATES_TEAM_NAME` env 또는 `tgates`로 최소 배너 생성·캐시) + **시작 멘트(greeting) 출력**(`team.config.json`의 `team.greeting` 있으면) ② Claude 어댑터 `sync(mode="on")` ③ `.tgates-active` 마커 기록 ④ upstream **fetch만** 자동 — behind면 `teammode update` 안내(merge 절대 자동 금지; 미설정·오프라인·git 아님이면 조용히 패스, on을 막지 않음). exit 0.
- **off**: 어댑터 `sync(mode="off")` → `.tgates-active` 마커 삭제 → **끝맺음 말(farewell) 출력**(`team.config.json`의 `team.farewell` 있으면, 없으면 "상태 저장됨"). exit 0.
- **active 마커**: 팀 모드 활성 상태 표식 = 팀 루트의 `.tgates-active`(빈 파일). 런타임 훅은 이 마커가 있을 때만 동작한다.
...
reference: `install.bootstrap`이 ①~⑦ 전부 오케스트레이션. `install_lib`이 순수/주입 가능 함수(테스트가 호스트 무접촉). ⑦ verify는 엔진을 subprocess로 호출하되 **env 화이트리스트**(`PATH/HOME/LANG/…`)로 ambient `TEAMMODE_HOME`/`TGATES_HOME` 누수 차단(P1 이중 방어).
...
추가로 **골든 시나리오 5종**(켜기 → 컨텍스트 조회 → 이슈 생성 → 세션로그 작성 → 끄기)을 실환경 통과(v0.1 수동 체크리스트, 자동화는 kit 후속). reference 시나리오 = `01-on-banner`·`02-context-injection`·`03-issue-create`·`04-log-accumulate`·`05-off-persist`. ⚠️ `03-issue-create`는 `issue create` 동사를 기대하나 reference 엔진에 미구현(exit 127) → 의도된 RED(서비스 슬롯 L2) — 부록 A.
...
- **conformance 시나리오 03(`issue create`)**: 엔진 미구현 동사 → exit 127(의도된 RED, 서비스 슬롯 L2).

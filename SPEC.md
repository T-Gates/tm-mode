...
2. **등록 훅 커맨드는 반드시 normalize 경유로 배선**: `<python> agents/<name>/normalize.py <script> [args]`. 공통 스크립트 직접 등록 금지.
...
⑥ env         런타임 훅용 TEAMMODE_HOME 을 셸 프로파일에 멱등 1줄 주입(§4.8)
...
- 런타임 훅용 팀 루트 env를 셸 프로파일에 **멱등 1줄** 주입(§1.2). 변수명 = **`TEAMMODE_HOME`**(런타임 훅 코드와 일치). ⚠️ 의도적 호출(install/on/off)은 env 무신뢰 — env는 런타임 훅 전용.
- 셸 감지(`$SHELL`): bash→`.bashrc`/`export`, zsh→`.zshrc`/`export`, fish→`.config/fish/config.fish`/`set -gx`. 미지원 셸은 경고만(비치명). 멱등 마커(`# teammode (env injection, §9)`)로 중복 판정 — 같으면 무변경, 팀루트 바뀌면 그 줄만 교체.
...
| I4b | `--settings <격리>` 지정 | 실 `~/.claude/settings.json` **및 실 셸 프로파일(env 주입) 무접촉**, 격리 경로에만. `--settings`=env 격리 권위(`--yes` 와 와도 격리 우선). 실 env 주입은 `--settings` 없는 `--yes` 에서만 |

...
- Auto-committed session logs again publish through the bounded foreground path: a non-fast-forward rejection now reaches fetch → `rebase --autostash` → re-push instead of being stranded behind the plain-push-only worker. Dirty files that overlap upstream changes defer rebase. Post-rebase verification tracks Git's exact created-autostash object, and a conflicted autostash apply rolls the branch back before restoring that exact object with its backup preserved. The 70-second auto-commit cap contains a 35-second publication budget plus explicit cross-platform rollback, pending validation, lock, and fsync headroom. Separately, the 60-second SessionStart cap reserves its final 10 seconds for context output and skips optional refresh/diagnostic Git work when its shared cutoff is exhausted. Session-start restarts the fallback worker only after reconcile, and `tm off` keeps its hooks active until both `ahead=0` and an available, empty pending ledger prove publication.
- Failed publication remains non-fatal and records a redacted sync warning plus fallback ledger. The ledger is now branch-bound and multi-entry, binds a failed commit to its captured branch/HEAD even if checkout changes before fallback recording, stores the failure HEAD so a branch rename can be migrated only when the old ref is gone and the HEAD still matches, preserves other-branch retries across checkout changes, and uses locked compare-and-delete. A v0.1.3 branchless marker is auto-bound only when every unpublished commit is an auto-committed canonical session log; ambiguous or ordinary-code history is preserved without pushing. Warning cleanup uses the same lock plus a file snapshot CAS so a concurrent failure cannot be erased. Hook paths are validated as repo-local literal pathspecs, and local state is written atomically with private POSIX modes and fsync durability.
...
- async fallback (#45, superseded by the foreground recovery fix above): failed foreground publication records an XDG pending ledger and detaches a per-team `push-worker` for drain/retry with **plain-push-only** history safety. Session-start recovery, pending-age reminders, and immediate prior-pending warnings keep failures visible; the worker is no longer the owner of every successful push.

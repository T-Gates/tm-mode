...
# A안(상수 하드코딩) — 초기 단계라 config 필드 대신 고정(은수 결정 2026-06-17).
...
def bootstrap(opts: il.Options, *, home: Path, python_version,
    ...
    if _similar:
        err(f"[error] member name '{member_name}' is too similar to existing {_similar} "
            f"(AI confusion risk — junhyun↔junhyung case). Use a more distinct slug "
            f"(e.g. junhyun→xian). If truly intended, add it to members.md manually.")
        return 3

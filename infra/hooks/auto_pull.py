#!/usr/bin/env python3
"""auto_pull — 스로틀 기반 안전 pull 순수 함수 모듈.

설계:
  SessionStart 등 안전한 호출자가 팀 레포 최신화를 요청할 때,
...
철칙(실패 무해)의 구현 원칙:
  - 외부에 노출되는 do_pull/auto_pull 은 **절대 예외를 전파하지 않는다**. 모든 실패는
    결과 객체(ok=False)로 표현된다. 호출자의 작업을 막을 수 있는 경로 0.
"""
...
def _record_pull_time(state_path: str, now: float) -> None:
    ...
    try:
        ...
        pass  # 상태 기록 실패는 작업을 막지 않는다 — 다음 안전 호출에서 재시도될 뿐
...
def auto_pull(team_root: str, state_path: str, now: float,
    ...
    **철칙**: 어떤 단계에서 무슨 일이 나도 예외를 전파하지 않는다 — 사용자 프롬프트
    ...
    try:
        ...
        # 때 연속 호출마다 (최대 timeout 초) pull 을 재시도하며 작업에 세금을 물린다.

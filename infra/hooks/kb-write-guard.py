...
def _deny_edit_mutex(reason: str, lang: str = "ko") -> None:
    ...
    sys.stderr.write(_t(
        "hook_edit_mutex_stderr_blocked", lang,
        "[teammode] 파일 편집 보류: {reason}", reason=reason) + "\n")


def _begin_edit_mutex(root: str, data: dict, lang: str) -> bool:
    """Reserve this exact tool call after every governance check passes."""
    if _git_ops is None:
        if not os.path.exists(os.path.join(root, ".git")):
            return True  # non-repository fixtures cannot run Git reconciliation
        reason = _t(
            "hook_edit_mutex_deny_unavailable", lang,
    ...
    if not token:
        if not _git_ops.is_git_worktree(root):
            return True
        reason = _t(
            "hook_edit_mutex_deny_identity", lang,
            "도구 식별자가 없어 안전한 자동 정합을 보장할 수 없습니다. "
    ...
    if acquired:
        return True
    reason = _t(
        "hook_edit_mutex_deny_busy", lang,
        "다른 세션의 정합 작업과 겹쳐 파일 편집을 시작하지 않았습니다. "
        "잠시 후 다시 시도하세요: {detail}", detail="edit mutex busy")
    _deny_edit_mutex(reason, lang)
    return False
...
def main() -> int:
    ...
    if not needs_unlock:
        return 0 if _begin_edit_mutex(root, data, lang) else 2

    # ── 5. memory/ 파일 포함 → unlock 플래그 확인 ──
    if _is_unlock_valid(root, data):
        # 스킬이 플래그를 세운 구간 → governance 통과 후 edit mutex 등록.
        return 0 if _begin_edit_mutex(root, data, lang) else 2

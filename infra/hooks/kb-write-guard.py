...
세션 ID 매칭(필수): CLAUDE_SESSION_ID(또는 CLAUDE_CODE_SESSION_ID)가 없으면 → deny(fail-closed).
...
def _is_unlock_valid(team_root: str) -> bool:
    ...
    세션 ID(CLAUDE_SESSION_ID 또는 CLAUDE_CODE_SESSION_ID)가 없으면
    fail-closed — 세션 없는 환경에서는 unlock 플래그를 신뢰할 수 없으므로 deny.

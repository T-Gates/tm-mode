...
────────────────────────────────────────────────────────────────────────────
⚠️ 빌드 안전 핵심 — `.acme-active` 가드 (L2-G):
  팀 루트에 `.acme-active` 마커가 없으면(teammode off) **즉시 no-op exit 0**.
...
def _team_root() -> str:
    ...
    가 아닌 쓰기 훅이지만, `.acme-active` 가드가 활성 팀 루트에서만 동작을 허용하므로
...
def main() -> int:
    ...
    # ── 1. 빌드 안전 핵심: .acme-active 없으면 즉시 no-op ──

...
```bash
python infra/teammode.py memory unlock begin --root .
```
...
```bash
python infra/teammode.py memory unlock end --root .
```
...
python infra/teammode.py memory route upsert \
  --root . --path extras/ --desc "팀 과정 관련 정보" --author <현재사용자>
# 해제: memory route remove --root . --path extras/ --author <현재사용자>
```

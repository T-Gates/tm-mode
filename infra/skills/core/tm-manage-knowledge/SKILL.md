---
name: tm-manage-knowledge
description: Use when adding, updating, or deleting knowledge in the team knowledge base. Triggers on "지식 추가해", "지식 수정", "지식 삭제", "KB 업데이트", "메모리에 저장".
---

# tm-manage-knowledge — 팀 지식 베이스 CRUD

## Overview

`memory/` 하위 지식 파일의 추가/수정/삭제를 처리한다.
**판단은 이 스킬이, 기계는 엔진 `knowledge` 동사가** 담당한다(구현 깊이 B).
스킬은 직접 파일을 쓰거나 INDEX를 편집하지 않는다 — 반드시 엔진 동사를 경유한다.

## When to Use

- "지식 추가해", "KB에 저장", "메모리에 저장"
- "~ 수정해", "~ 업데이트해" (지식 파일 대상)
- "~ 삭제해", "~ 지워" (지식 파일 대상)
- 대화에서 도출된 내용을 지식 베이스에 반영할 때
...
모든 지식 파일에 YAML frontmatter(엔진이 자동 스탬프):
...
대상 폴더는 아래 중 하나여야 한다:
- `product/`, `team/`, `team/decisions/`, `extras/`
...
> **unlock 플래그에 대한 정직한 설명**: 엔진 `knowledge` 동사는 별도 프로세스 `open()` 이라
...
`CLAUDE_SESSION_ID` 가 없으면 훅이 fail-closed 로 deny 하므로, 세션ID 없이 스킬을 실행하면 안 된다.
...
if not session_id:
    raise RuntimeError("CLAUDE_SESSION_ID 가 없음 — 훅이 deny 하므로 unlock 불가")
...
if not team_root or not session_id:
    raise RuntimeError("TEAMMODE_HOME 또는 CLAUDE_SESSION_ID 가 없음")
rh = hashlib.sha1(team_root.encode()).hexdigest()[:8]
suffix = f"{rh}-{session_id}"
xdg = os.environ.get("XDG_STATE_HOME")
...
**추가/수정:**
```bash
python infra/teammode.py knowledge write \
  --root . \
  --folder <폴더> \
  --filename <파일명.md> \
  --content "<내용>" \
...
**삭제 (삭제 전 사용자 재확인 필수):**
```bash
python infra/teammode.py knowledge delete \
  --root . \
  --path <memory/상대경로> \
  --author <현재사용자>
```
...
- 세션 로그 관리 (→ 훅 자동)
- 회의록 관리 (→ tm-context)
- 지식 읽기/로드 (→ tm-knowledge)
- 이슈 트래커 / 문서 도구 / 일정 관리 API 호출
- 파일 직접 편집 (반드시 knowledge 동사 경유)
...
- tm-knowledge 스킬이 이 INDEX로 "무엇이 어디 있는지"를 파악한다

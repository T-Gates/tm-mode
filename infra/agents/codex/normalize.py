...
⚠️ 주의(스펙 02 부록 B / 초안 §12 미결): Codex 훅 입력 JSON 은 문서상
`hook_event_name`, `tool_name`, `tool_input`, `prompt` 형태를 사용한다. 세션 tool-call
기록처럼 top-level `name`/`input` 으로 들어오는 apply_patch 도 보수적으로 지원한다.
2026-06-21 실 hook stdin 캡처 기준 apply_patch는 `tool_input.command`에 patch 문자열을
담는다. normalize는 command/patch/input 문자열의 파일 헤더를 files[]로 변환한다.
"""

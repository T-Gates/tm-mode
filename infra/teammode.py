...
def cmd_route(team_root: Path, sub_action: str | None,
    ...
    엔진 불변(memory write/delete 와 동일 규약):
      ...
      - 엔진이 python 직접 write — 훅 경유가 아니라 별도 프로세스 open() 이다.
    """
    index_path = team_root / "memory" / "INDEX.md"
    lang = _i18n.team_lang(str(team_root))
...
def cmd_pull(team_root: Path) -> int:
    """팀 레포 main을 fetch → pull --rebase → push로 즉시 동기화한다.

    실패는 비치명(우아한 축소): git 아님·오프라인·non-main·rebase 충돌·타임아웃은
    exit 1 + 안내로 표면화하고 last-sync-error는 core가 관리한다. branch 생성이나
    checkout, upstream 설정은 하지 않는다.
    """
    lang = _i18n.team_lang(str(team_root))
    result = _git_ops.sync_main(str(team_root))
    detail = _git_ops.sanitize_git_detail(result.detail or "")
    if result.ok:
...
def cmd_commit(team_root: Path, message: str, push: bool,
    ...
    실패 무해(우아한 축소): 변경 없음·git 아님·동기화 실패에도 크래시하지 않는다.
    동기화 실패는 로컬 커밋을 되돌리지 않고 core의 last-sync-error로 표면화한다.
    ...
    display_detail = _git_ops.sanitize_git_detail(result.detail or "")
    ...
    if result.committed:
        print(_t(
            "cmd_commit_sync_failed", lang,
            "tm-mode commit — 로컬 커밋은 보존했지만 main 동기화에 "
            "실패했습니다: {detail}", detail=display_detail), file=sys.stderr)
        return 0
    # 변경 없음/git 아님 등 — 비치명. 작업을 막지 않되 사유를 알린다.
    print(_t("cmd_commit_skipped", lang, "tm-mode commit — 건너뜀(비치명): {detail}",
             detail=display_detail), file=sys.stderr)
    return 1

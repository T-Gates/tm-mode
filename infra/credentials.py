...
B-3 결정(팀): **각 멤버가 자기 토큰을 직접 입력한다.** 팀 토큰 자동공유 메커니즘은
...
저장 위치(B-3): `$XDG_DATA_HOME/teammode/credentials/default.json`
  (단일 금고 — 멀티팀 미지원, 2026-06-21. 팀명에 묶이지 않아 개명 안전.)
...
# 단일 금고 파일 — 팀명으로 쪼개지 않는다(멀티팀 미지원, 2026-06-21 팀 결정).
# team 인자는 호출부 시그니처 호환을 위해 남기되 파일명에는 쓰지 않는다 →
# team.name 을 언제 바꿔도 금고 키가 안 변한다(개명 안전). 멀티팀이 필요해지면
# 그때 team 별 파일명으로 되살린다("그때 가서 생각" — 미리 복잡도 지지 않음).
_VAULT_FILENAME = "default.json"


def _vault_path(team: str | None = None) -> Path:
    return credentials_dir() / _VAULT_FILENAME


def migrate_legacy_vault(old_team: str) -> bool:
    """팀명-키 금고(`<old_team>.json`)를 단일 금고(`default.json`)로 1회 이전한다.

    단일파일 전환(2026-06-21) 전에 L2 를 연결한 팀의 토큰을 보존한다. 목적지(default.json)가
    이미 있으면 건드리지 않는다(멱등 — 덮어쓰기로 토큰 유실 방지). old_team 이 비었거나
    식별자로 부적합하면 no-op. 호출은 install/on 등 team.name 을 아는 지점에서 1회.
    """
    if not old_team:
        return False
    try:
        old = credentials_dir() / f"{_check_ident(old_team, 'team')}.json"
    except ValueError:
        return False
    new = _vault_path()
    # 레거시가 symlink 면 이전하지 않는다 — rename 은 symlink 자체를 옮겨 default.json 이
    # symlink 가 되고, 이후 O_NOFOLLOW 금고 접근이 영구·무음 불능이 된다(이 모듈의
    # symlink 하드닝 철학과 일관). 의심스러운 레거시는 건드리지 않고 no-op.
    if old.is_symlink():
        return False
    if old.exists() and old.resolve() != new.resolve() and not new.exists():
        _secure_dir(new.parent)
        old.rename(new)
        new.chmod(_FILE_MODE)  # 이전 후 0600 재단언(_write_vault 와 동일 보장 — rename 이
        #                        권한을 보존하더라도 명시적으로 못박는다).
        return True
    return False
...
def _read_vault(team: str | None) -> dict:
...
def _write_vault(team: str | None, data: dict) -> Path:

...
def _make_upstream(tmp_path: Path, notice_text: str) -> Path:
    ...
    CI 러너는 init.defaultBranch 가 설정돼 있지 않아 `git init` 이 "main" 대신
    "master"(또는 그 외 기본값)를 만든다(로컬 Mac 은 전역 설정 때문에 우연히
    통과했었다 — 적대검수 발견). bare repo 의 HEAD symref 를 생성 직후 명시적으로
    refs/heads/main 으로 고정해 이후의 모든 clone 이 러너 기본값과 무관하게 "main"
    을 일관되게 체크아웃하게 한다.
    """
...
def _push_new_notice(tmp_path: Path, upstream: Path, notice_text: str) -> None:
    ...
    브랜치명을 명시로 고정(_make_upstream 의 HEAD symref 수정과 동형) — clone 직후
    "main" 을 명시적으로 checkout 해서 러너의 기본 브랜치명 설정과 무관하게 만든다.
    """

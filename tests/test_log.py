...
def test_log_creates_file_with_frontmatter(tmp_path):
    r = _run(tmp_path, "log", "--author", "eunsu", "--text", "첫 항목",
             "--now", "2026-06-13T14:00:00+09:00")
    assert r.returncode == 0, r.stderr
    files = _log_files(tmp_path, "eunsu")
    assert len(files) == 1
    assert files[0].name == "2026-06-13.md"
    content = files[0].read_text(encoding="utf-8")
    assert content.startswith("---\n")
    assert "author: eunsu" in content
...
def test_log_appends_same_day_single_file(tmp_path):
    _run(tmp_path, "log", "--author", "eunsu", "--text", "첫 항목",
         "--now", "2026-06-13T10:00:00+09:00")
    r = _run(tmp_path, "log", "--author", "eunsu", "--text", "둘째 항목",
    ...
    assert content.count("author: eunsu") == 1


def test_log_different_day_new_file(tmp_path):
    _run(tmp_path, "log", "--author", "eunsu", "--text", "어제",
         "--now", "2026-06-12T10:00:00+09:00")
    _run(tmp_path, "log", "--author", "eunsu", "--text", "오늘",
         "--now", "2026-06-13T10:00:00+09:00")
    files = _log_files(tmp_path, "eunsu")
...
def test_log_before_six_writes_previous_day_file(tmp_path):
    ...
    r = _run(tmp_path, "log", "--author", "eunsu", "--text", "새벽작업",
...
def test_log_six_oclock_writes_same_day(tmp_path):
    r = _run(tmp_path, "log", "--author", "eunsu", "--text", "아침작업",
...
def test_log_dawn_appends_to_previous_day(tmp_path):
    ...
    _run(tmp_path, "log", "--author", "eunsu", "--text", "밤작업",
         "--now", "2026-06-12T23:00:00+09:00")
    _run(tmp_path, "log", "--author", "eunsu", "--text", "새벽이어서",
...
def test_log_text_with_newlines_summary_is_first_line_only(tmp_path):
    ...
    _run(tmp_path, "log", "--author", "eunsu",
         "--text", "첫줄요약\n둘째줄상세\n셋째줄", "--now", "2026-06-13T10:00:00+09:00")
    content = _log_files(tmp_path, "eunsu")[0].read_text(encoding="utf-8")
    fm = content.split("---\n")[1]   # frontmatter 블록
...
def test_log_requires_root(tmp_path):
    ...
    r = subprocess.run(
        [sys.executable, str(ENGINE), "log", "--author", "eunsu", "--text", "x"],
...
def test_log_requires_text(tmp_path):
    r = _run(tmp_path, "log", "--author", "eunsu", "--now", "2026-06-13T10:00:00+09:00")
    assert r.returncode != 0
...
def test_log_append_preserves_existing_content(tmp_path):
    _run(tmp_path, "log", "--author", "eunsu", "--text", "MARKER_ONE",
         "--now", "2026-06-13T10:00:00+09:00")
    before = _log_files(tmp_path, "eunsu")[0].read_text(encoding="utf-8")
    _run(tmp_path, "log", "--author", "eunsu", "--text", "MARKER_TWO",
         "--now", "2026-06-13T11:00:00+09:00")
    after = _log_files(tmp_path, "eunsu")[0].read_text(encoding="utf-8")
...
def test_log_without_now_uses_real_time(tmp_path):
    r = _run(tmp_path, "log", "--author", "eunsu", "--text", "지금")
    assert r.returncode == 0
    assert len(_log_files(tmp_path, "eunsu")) == 1


# ── 멀티 author 격리 ──

def test_log_separate_authors_separate_dirs(tmp_path):
    _run(tmp_path, "log", "--author", "eunsu", "--text", "A",
         "--now", "2026-06-13T10:00:00+09:00")
    _run(tmp_path, "log", "--author", "junhyung", "--text", "B",
         "--now", "2026-06-13T10:00:00+09:00")
    assert len(_log_files(tmp_path, "eunsu")) == 1
    assert len(_log_files(tmp_path, "junhyung")) == 1
    assert "A" in _log_files(tmp_path, "eunsu")[0].read_text(encoding="utf-8")
    assert "B" in _log_files(tmp_path, "junhyung")[0].read_text(encoding="utf-8")

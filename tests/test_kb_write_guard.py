...
검증 목록:
  ...
  - .acme-active 없으면 → no-op (exit 0, 빌드 안전).
...
def test_team_root_ignores_env_and_uses_file_location(tmp_path):
    ...
    훅이 env 를 신뢰하면 다른 경로의 .acme-active 를 보거나 no-op;
    __file__ 을 따르면 tmp_path 의 .acme-active 를 보고 deny 한다.
    """
    root = tmp_path / "team"
    root.mkdir()
    (root / "memory").mkdir()
    _active(root)  # 자기 repo에 .acme-active 생성

    # 다른 inactive repo (오염 env)
    other = tmp_path / "other-inactive-repo"
    other.mkdir()
    # other 에는 .acme-active 없음 → env 신뢰하면 no-op(exit 0) 됨
    ...
    # env 를 신뢰하면 other 의 .acme-active 를 보고 no-op(0); __file__ 따르면 deny(2)
    assert proc.returncode == 2, (

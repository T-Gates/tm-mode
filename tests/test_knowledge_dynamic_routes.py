...
def test_extras_not_statically_allowed(tmp_path):
    ...
    r = _run(root, "memory", "write", "--folder", "extras",
             "--filename", "x.md", "--content", "x",
             "--author", "test", "--weight", "📎")
    assert r.returncode == 2, "extras 가 아직 정적 허용됨 (하드코딩 잔존)"
    assert "허용되지 않습니다" in r.stderr


def test_extras_allowed_when_registered(tmp_path):
    """팀이 extras/ 를 루트 INDEX 에 등재하면 동적 허용으로 write 가능 (우리 인스턴스 경로)."""
    root = tmp_path / "team"
    root.mkdir()
    _init_git(root)

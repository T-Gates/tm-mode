...
def test_context_collects_member_summary(tmp_path):
    _write_index(tmp_path)
    _write_log(tmp_path, "jane-doe", "2026-06-13", "오늘요약내용")
    r = _run(tmp_path, "context")
...
def test_context_picks_most_recent_workday_file(tmp_path):
    _write_index(tmp_path)
    _write_log(tmp_path, "jane-doe", "2026-06-10", "오래된요약")
    _write_log(tmp_path, "jane-doe", "2026-06-13", "최신요약")
    r = _run(tmp_path, "context")
...
def test_context_multiple_members(tmp_path):
    _write_index(tmp_path)
    _write_log(tmp_path, "jane-doe", "2026-06-13", "Jane작업")
    _write_log(tmp_path, "jonathon", "2026-06-13", "Jonathon작업")
    r = _run(tmp_path, "context")
    assert "jane-doe" in r.stdout and "jonathon" in r.stdout
...
def test_context_json_mode_parses(tmp_path):
    _write_index(tmp_path)
    _write_log(tmp_path, "jane-doe", "2026-06-13", "제이슨요약")
    ...
    assert jane-doe["summary"] == "제이슨요약"
    assert jane-doe["date"] == "2026-06-13"
...
def test_context_ignores_non_log_md_files(tmp_path):
    ...
    _write_log(tmp_path, "jane-doe", "2026-06-13", "진짜요약")
    r = _run(tmp_path, "context")

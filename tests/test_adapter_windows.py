...
WIN_PY_BS = r"C:\Users\gabur\AppData\Local\Programs\Python\Python312\python.exe"
...
def test_build_command_no_backslash_in_windows_path(env, monkeypatch):
    ...
    ad.normalize_path = type(ad.normalize_path)(
        r"C:\Users\gabur\team\infra\agents\claude\normalize.py")
    cmd = ad.build_command({"script": "auto-commit.py"})
    assert "\\" not in cmd, f"커맨드에 백슬래시 노출(bash escape 깨짐): {cmd!r}"
    # slash 경로가 들어있어야
    assert "C:/Users/gabur/AppData/Local/Programs/Python/Python312/python.exe" in cmd
    assert "C:/Users/gabur/team/infra/agents/claude/normalize.py" in cmd
...
def test_windows_command_survives_bash_escape(env):
    ...
    assert escaped == cmd
    # 실측 깨짐 문자열이 나오면 안 됨
    assert "C:UsersgaburAppData" not in escaped
...
def test_is_owned_migrates_legacy_backslash_command(env):
    ...
    ad.normalize_path = type(ad.normalize_path)(
        r"C:\Users\gabur\team\infra\agents\claude\normalize.py")
    legacy = (r'C:\Users\gabur\AppData\python.exe '
...
def test_codex_inherits_slash_normalization(env, tmp_path):
    ...
    ad.normalize_path = type(ad.normalize_path)(
        r"C:\Users\gabur\team\infra\agents\codex\normalize.py")
    cmd = ad.build_command({"script": "auto-commit.py"})

...
케이스:
  1. session-log-remind — UserPromptSubmit wire + TEAMMODE_MEMBER env, 스테일 로그
     상태 → additionalContext ≤3줄(compact 계약) + exit 0.
  2. session-start — SessionStart wire + 활성 팀루트 → 유효 JSON,
     additionalContext 존재, exit 0.
  3. 미지/변형 wire — exit 0, stdout 무배출(fail-open-quietly 계약).

격리 규약(test_codex_integration_faults 준수):
  - infra/ 트리를 tmp 팀루트로 통째 복사해 실행 — __file__ 기준 팀루트 훅이
    tmp 루트를 보게 한다(실호스트 무접촉).
  - env 스크럽: CLAUDE_*_SESSION_ID / TEAMMODE_HOME / TEAMMODE_MEMBER 제거.
  - XDG_STATE_HOME·TMPDIR 을 tmp 로 고정(remind 상태 격리).
...
# ═══════════════════════════════════════════════════════════════════════
# 1. session-log-remind — compact 계약 (UserPromptSubmit, 멤버 env)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# 2. session-start — 활성 팀루트 맥락 주입
# ═══════════════════════════════════════════════════════════════════════

def test_session_start_injects_context_via_codex_normalize(tmp_path):
    """SessionStart wire → 유효 JSON + additionalContext + exit 0."""
    root = _make_team_root(tmp_path)
...
# ═══════════════════════════════════════════════════════════════════════
# 3. 미지/변형 wire — fail-open-quietly
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# 4. session-start 엔진 업데이트 알림 — Codex 경로에도 동일하게 흐르는지
# ═══════════════════════════════════════════════════════════════════════
#

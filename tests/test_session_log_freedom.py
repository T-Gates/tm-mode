...
class TestIsOwnSessionLog:
    ...
    def test_own_file_returns_true(self, monkeypatch, tmp_path):
        """TEAMMODE_MEMBER=eunsu + sessions/eunsu/x.md → True."""
        _make_sessions_dir(tmp_path, "eunsu")
        fn = self._fn(monkeypatch, "eunsu", tmp_path)
        fp = str(tmp_path / "memory" / "team" / "sessions" / "eunsu" / "x.md")
        assert fn(fp, str(tmp_path)) is True

    # ── False 케이스 ────────────────────────────────────────────────────────

    def test_other_member_returns_false(self, monkeypatch, tmp_path):
        """남의 세션로그(sessions/junhyung/x.md) → False."""
        _make_sessions_dir(tmp_path, "eunsu")
        _make_sessions_dir(tmp_path, "junhyung")
        fn = self._fn(monkeypatch, "eunsu", tmp_path)
        fp = str(tmp_path / "memory" / "team" / "sessions" / "junhyung" / "x.md")
    ...
    def test_symlink_sessions_dir_returns_false(self, monkeypatch, tmp_path):
        ...
        # sessions/eunsu 를 decisions 디렉토리로 symlink
    ...
    def test_unicode_member_name_returns_false(self, monkeypatch, tmp_path):
        """멤버명 유니코드("은수") → False (슬러그 화이트리스트 위반)."""
        member = "은수"
...
class TestFindSimilarNames:
    ...
    def test_close_edit_distance_detected(self):
        """junhyun vs junhyung (편집거리 1) → 유사 목록에 포함."""
        result = il.find_similar_names("junhyun", ["junhyung", "eunsu"])
        assert result == ["junhyung"]

    def test_dissimilar_name_not_detected(self):
        """xian vs [junhyung, eunsu] (편집거리 큼) → 빈 리스트."""
        result = il.find_similar_names("xian", ["junhyung", "eunsu"])
        assert result == []

    def test_identical_name_excluded(self):
        """동일 이름(eunsu vs [eunsu]) → 제외(UNIQUE 처리는 register_member 몫)."""
        result = il.find_similar_names("eunsu", ["eunsu"])
        assert result == []

    def test_prefix_similarity_detected(self):
        """공통 프리픽스 유사("jun" vs ["junhyung"]) → 유사로 검출.

        "jun" 의 80% = 2.4 → ceil 2. "junhyung" 과의 공통 프리픽스 "jun" = 3 >= 2.
        편집거리 = 5(> max_distance=2)이지만 프리픽스 조건으로 잡힌다.
        """
        result = il.find_similar_names("jun", ["junhyung"])
        assert "junhyung" in result

    def test_empty_existing_returns_empty(self):
        """기존 이름 목록이 비어있으면 항상 빈 리스트."""
        result = il.find_similar_names("eunsu", [])
        assert result == []

    def test_exact_match_among_many_excluded(self):
        """여러 이름 중 동일 이름은 제외, 유사 이름만 반환."""
        result = il.find_similar_names("eunsu", ["eunsu", "eunsy"])
        assert "eunsu" not in result
        assert "eunsy" in result
...
class TestInjectMemberEnvSettings:
    ...
    def test_first_injection_returns_true_and_sets_member(self, tmp_path):
        ...
        result = il.inject_member_env_settings(settings, "eunsu")
    ...
    def test_existing_env_keys_preserved(self, tmp_path):
        ...
        il.inject_member_env_settings(settings, "eunsu")
        data = json.loads(settings.read_text(encoding="utf-8"))
    ...
    def test_different_value_reinject_returns_true(self, tmp_path):
        ...
        settings = tmp_path / "settings.json"
        il.inject_member_env_settings(settings, "eunsu")
        result = il.inject_member_env_settings(settings, "junhyung")
        assert result is True
        data = json.loads(settings.read_text(encoding="utf-8"))
        assert data["env"]["TEAMMODE_MEMBER"] == "junhyung"

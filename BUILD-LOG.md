...
Jane의 세션로그(acme-toolkit, 2026-06-12, 본인 작성분 — 무수정)에서 확인된 사고:
- **원인**: 슬라이스 2 초기 `verify` 실행 시(env 격리 패치 *이전*), 빌드 에이전트 셸에 상속된 ambient `LEGACY_TOOL_HOME`(=실 acme-toolkit)이 conformance 러너로 새어들어, `05-off-persist` 시나리오의 `off` 동작이 **실 toolkit의 `.acme-active` 플래그를 실삭제**. banner.txt 도 같은 경로로 잠시 건드림(읽기만, 무수정·복구됨).
- **이미 적용된 수정**(슬라이스 2 커밋 a81b6b0): `SubprocessEngine.run`이 subprocess env의 `LEGACY_TOOL_HOME`을 run root로 명시 주입 → ambient 무시. 검증: ambient `LEGACY_TOOL_HOME=실toolkit` set 상태로 verify 돌려도 실 `.acme-active` 생존·banner는 격리 cwd 생성 확인.
- **복구 조치**: 내 초기 verify가 삭제한 실 toolkit `.acme-active`(런타임 마커, gitignored)를 `touch`로 재생성 — 사고 전 상태 복원. Jane 세션로그 파일은 본인 작성분이라 손대지 않음.
...
- **호출처 동기화**:
  ...
  - 스펙 01 §1.2(팀 루트 정의)·§2.4(쓰기 위치)에 "엔진/어댑터는 명시 인자로만, env 비신뢰, 미지정 시 에러" 반영. reference 변수명도 `LEGACY_TOOL_HOME`→`TEAMMODE_HOME` 정정.
...
- **적대적 자기검수(별도 시각 — "ambient env로 호스트 오염 재현 시도", /tmp 피해자만 사용)**: 4종 공격 — ① ambient `TEAMMODE_HOME=피해자`+`off --root 격리` → 피해자 마커 생존 ② `--root` 없는 `off` → 에러 종료·cwd 무접촉 ③ `--settings`/`--install` 없는 `on` → 거부·마커 미생성 ④ 플래그 순서 뒤섞기(`--settings ... --root ...`) → 정상 바인딩·피해자 무변. **전부 차단**. 실 `acme-toolkit` 무접촉(피해자는 전부 `mktemp -d`). 실 `~/.claude/settings.json` 누수 0 + conftest 가드 통과.
- 재검수: 구현 결함 0건. "수정할 내역 없음".
...
> 설계(Jane 새벽 합의): UserPromptSubmit 마다 팀 레포를 최신화하되 스로틀로 과부하 방지, **실패는 절대 작업 차단 금지(철칙)**. 슬라이스 T(on 시 upstream fetch)와 별개 축 — 이건 팀 레포 자체의 상시 ff-pull.
...
- **훅 통합 방식 = session-log-remind 인라인 호출(별도 manifest 엔트리 아님)**. 근거: ① 설계가 "pull 이 리마인드보다 **먼저**"를 요구 — 한 프로세스 안에서 호출하면 순서가 결정적(별도 settings.json 엔트리는 등록 순서 의존이라 취약). ② normalize subprocess 1회 절약. ③ 기존 UserPromptSubmit 훅이 이미 `.acme-active` 게이트·TEAMMODE_HOME 해석을 하므로 재사용. `_maybe_auto_pull` 는 `.acme-active` 활성 시 리마인드 age 계산 **직전**에 호출되고, 어떤 예외도 삼킨다(철칙).
...
**결정/근거**
- **fetch 만 자동, merge 절대 금지(Jane 합의의 핵심 안전)**: `on` 은 banner+sync+marker 후 `_maybe_notify_upstream` 호출 — upstream fetch(조용·실패무시·타임아웃) → count_behind → behind 면 upstream_changes(git log 원본) 알림. **merge 는 전혀 안 한다**. 적용은 명시적 `update` 동사로 분리(자동 적용이 워킹트리를 건드리는 사고 방지).
...
- **env 불신뢰(§10, P1)**: `_resolve_root` 은 --root 명시 우선, 미지정 시 cwd 가 팀표식(.git/team.config.json/memory) 가질 때만 cwd. ambient TEAMMODE_HOME/LEGACY_TOOL_HOME 절대 안 읽음.
...
- **banner 선기록(M4)**: memory/banner.txt 를 team.name 으로 — 엔진은 파일 있으면 그대로 읽어 무수정(env ACME_TEAM_NAME 우회 불요).
...
- **활성 게이트**: .acme-active 있을 때만 주입. 팀 루트 = TEAMMODE_HOME(런타임 훅이라 env 정당, session-log-remind 동일 근거).
...
- **무인 안전 철칙 준수**: 실 ~/.bashrc·~/.zshrc·~/.claude·실 git config 무접촉(전 슬라이스 e2e md5 불변 실증). env 불신뢰(ambient TEAMMODE_HOME/LEGACY_TOOL_HOME 무시, I4 회귀락). 실호스트 배선은 --yes/--settings 명시 게이트. 직렬 진행. 푸시 안 함(사람 몫).
- **L1 도달**: install.py 단독으로 빈 레포→메모리 스캐폴드+훅 배선+env+SessionStart 맥락주입까지(서비스 연결 L2 제외). 골든 7종으로 인수.
- **남은 항목(사람)**: 푸시/PR Jane 판단, 세션로그·계획서 반영.

---

## 🪟 W — Windows 네이티브 지원 (2026-06-15 새벽 착수)

> 기준선 301. 파이=Linux, 실 윈도우 없음 → Windows 분기는 **sys.platform/os.name 모킹 + subprocess(setx/reg) 모킹(runner 주입)** 으로 단위 테스트. 실 setx/reg 절대 미실행. 실 윈도우 검증은 Jane 내일(범위 밖).
...
> ⚠️ **실 윈도우 검증 미수행(모킹만)** — 실제 setx/reg 동작·새 프로세스 env 반영은 Jane 내일 native 윈도우에서 확인.
...
> ⚠️ **실 윈도우 검증 미수행(모킹만)** — 실제 윈도우 셸에서 절대경로 python + 따옴표 명령 실행은 Jane 내일 확인.
...
> ⚠️ **실 윈도우 검증 미수행(모킹/플랫폼주입만)** — 실제 윈도우 경로 해석은 Jane 내일 확인.
...
> ⚠️ **실 윈도우 검증 미수행(모킹/플랫폼주입만)** — 실 setx 레지스트리 영속·새 세션 env 반영·경로 해석은 Jane 내일 native 윈도우에서 확인.

...
| 범위 외 | 현행 코드(acme-toolkit) 마이그레이션·구현 노트는 스펙이 아니다 — 초안 §11.x 참조 |
...
**정규 서버명 (필수)**: MCP 서버의 등록 별칭은 환경마다 다르다(`slack-acme`, `claude_ai_Google_Calendar` 등). manifest는 `team.config.json`의 서비스 선언(§9)에 등장하는 **정규 서버명**(provider 식별자: `linear`, `slack`, `notion`, `google` 등)만 참조한다. 정규 서버명 → 실제 등록 별칭 매핑을 보장하는 것은 어댑터의 등록 시점 책임이다(§5.2).

...
|---|---:|---|---|---|
| `session-start.py` | ✅ `SessionStart` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.tgates-active` 부재, engine import/수집 실패면 no-op | 활성 시 **세션당 1회 auto-pull**(throttle·실패무해) 후 Claude additionalContext JSON stdout, 항상 exit 0 의도 |
| `session-log-remind.py` | ✅ `UserPromptSubmit` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.tgates-active` 부재면 no-op. 활성 시 로그 age/프롬프트 카운터 판정 (**pull 안 함** — 2026-06-17 P0 hook hang 수정으로 매 프롬프트 pull 을 SessionStart 1회로 분리) | 필요 시 additionalContext JSON stdout, 정상 exit 0. 단 counter 파일 write 실패는 현재 main에서 별도 catch하지 않음 |
...
| `auto-commit.py` | ✅ `PostToolUse` | 정규 JSON stdin | event/action 불일치, `.tgates-active` 부재, `git_ops` 부재, files 없음, 예외면 no-op | 정규 `files`만 commit 대상으로 넘기고 push 금지, 항상 exit 0 |
| `confirm-action.py` | ✅ `PreToolUse` | 정규 JSON stdin + 첫 argv marker | event 불일치, `.tgates-active` 부재, 대상 MCP 불일치, 사람 allow 신호 있으면 통과 | allow 없으면 deny JSON stdout + stderr, exit 2 |
...
- `off` 실행 순서:
  ...
  - 정상 완료 시 exit 0.
- 멱등성:
  - `on`은 반복 실행해도 같은 `.tgates-active` 빈 파일을 다시 쓰고 adapter sync를 다시 수행한다. 배너 파일이 이미 있으면 재생성하지 않는다. upstream은 매번 fetch를 시도할 수 있다.
...
- INDEX:
  ...
  - 파일이 아니면 빈 문자열이다.
- active 상태:
  - `<team_root>/.tgates-active` 존재 여부만 본다.

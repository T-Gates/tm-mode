...
| `session-log-remind.py` | ✅ `UserPromptSubmit` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.teammode-active` 부재면 no-op. 활성 시 로그 age/프롬프트 카운터 판정 (**pull 안 함** — 2026-06-17 P0 hook hang 수정으로 매 프롬프트 pull 을 SessionStart 1회로 분리) | 필요 시 additionalContext JSON stdout, 정상 exit 0. 단 counter 파일 write 실패는 현재 main에서 별도 catch하지 않음 |

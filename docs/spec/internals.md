...
│   ├── session-start.py          #   SessionStart main 동기화 + additionalContext 주입
...
│   ├── edit-lease-cleanup.py      #   unregistered no-op for old host configurations
...
- The default `enforcement` is `"advisory"`. In the current code, this field only strengthens the warning text on the Codex `sync()` path where `event is None`. That is, if an event is declared unsupported (`null`) in events.json and has `enforcement: "block"`, it prints `[warn] ... (block 강제 상실) → 비활성`. Codex supports six of the seven canonical events; `PostToolUseFailure` remains explicitly `null`, and the current manifest registers no failure cleanup hook. Claude `sync()` does not read `enforcement`.
...
The reference build declares four entries in `infra/hooks/manifest.json`. All declared scripts exist under `infra/hooks/`. `auto_pull.py` is a throttle helper and is not a manifest entry.
...
| `session-log-remind.py` | ✅ `UserPromptSubmit` | canonical JSON stdin | no-op if event mismatch, JSON parse failure, or `.teammode-active` missing. When active: member identification (TEAMMODE_MEMBER env first → single config fallback → fallback if absent) + age/counter decision based on my file mtime. check_reset: my file mtime changed or date changed (06:00 cutoff) → count=0 and no writing reminder. An independent `publication_count` advances while the existing persisted `last-sync-error` is nonempty, survives mtime/date changes, and resets when the error is absent. **The reminder hook itself performs no Git work; SessionStart owns main sync.** | when needed, **`hookSpecificOutput.additionalContext`+`systemMessage` JSON stdout** (propagated by normalize re-emission). Writing reminders emit strong(age≥1800 & 30-minute throttle) OR weak(count%5==0); every fifth unresolved-error prompt adds a sharing-not-confirmed notice without claiming a local commit exists. Both notices share the existing output and respect the system-message opt-out. Body defaults to **compact** (1-3 lines of dynamic state: Nth prompt, file path, offset + rule reference — the rule body is injected once by session-start via `_slog_rules.SESSION_LOG_RULES`) — opt back into the legacy long body (count/offset append kit) with `ux.session_log_remind.context_style:"full"`; also degrades to the long body if the rules module is missing (fail-to-verbose). Normal exit 0. State-file write failures are caught as OSError (safe). |
...
| `auto-commit.py` | ✅ `PostToolUse` | canonical JSON stdin | no-op on event/action mismatch, `.teammode-active` missing, `git_ops` missing, no valid repo-local files, or exception | validates canonical `files` as repo-local literal pathspecs and runs path-scoped `do_commit(push=True)`. The core holds one repository OS lock from add through commit and main sync (fetch → pull --rebase when behind → push). Sync failure preserves the local commit and records sanitized `last-sync-error`; no asynchronous publication state is created. Retries `index.lock` once after 1s, letting the core acquire the OS lock again, and always exits 0 |
| `edit-lease-cleanup.py` | ❌ compatibility only | ignored | no-op retained for already installed host configurations until normal hook wiring removes the old entry | always exits 0; no token, state-file, or Git operations |
...
**Simple main sync contract (0.4).** Auto-commit passes only normalized repo-local paths to `do_commit`. The core holds the existing repository OS lock (`_repo_sync_lock` / `_advisory_file_lock`) once from add through commit and optional main sync. Standalone `sync_main` and local-only commits use the same lock in the Git common directory, so linked worktrees serialize with each other. A committing caller invokes the private locked sync helper without reacquiring the non-reentrant lock. Main preflight remains read-only and precedes lock acquisition; main is checked again under the lock. The sync flow fetches `origin/main`, runs `pull --rebase origin main` when behind, and pushes `main` to `origin/main`. It never creates or checks out a branch, configures upstream tracking, force-pushes, or stashes unrelated dirty work. A sync failure preserves the scoped local commit and writes a credential-sanitized `last-sync-error`; verified synchronization clears that error. Lock acquisition uses the existing bounded repository-lock wait; operation deadlines and non-fast-forward retry limits remain unchanged.

There is no persistent edit token, marker lease, or 15-minute TTL. Existing edit-mutex marker files are ignored rather than migrated or deleted. The OS releases the advisory lock on handle close or process exit; the lock file itself may remain. Normal `run_git` timeouts still terminate and drain the Git process group. Abrupt parent death can leave an already running Git child alive because it has its own process group; releasing the parent's lock does not guarantee that orphan has stopped. SessionStart's separate context-deduplication lease and cooldown are unchanged.

The common guidelines injected at SessionStart tell the agent to respond to a session-start sync failure or a five-prompt sharing-failure notice with one attempt using the existing commands: inspect its own uncommitted session-log paths and use scoped `commit --paths … --push`, or use `pull --root` (the existing main sync flow) if already committed. Other members' files, code, and separately staged user changes are excluded. A continued failure is recorded for the next opportunity rather than retried in a loop. This is an agent instruction; the reminder hook itself performs no Git work.
...
- **Codex lifecycle support**: Codex supports six of the seven canonical events in events.json. `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, and `SubagentStop` map to the same names; `PostToolUseFailure` remains explicitly `null`; the current manifest registers no failure cleanup hook. Blocking hooks with `enforcement: block`, such as `confirm-action.py`, are registered as `[[hooks.PreToolUse]]` in `config.toml`, so exit-2 blocking takes effect. Actual Codex hook input has the shape `tool_name`/`tool_input` (or top-level `name`/`input`), and apply_patch carries the patch string in `tool_input.command` (captured 2026-06-21) — normalize converts file headers into canonical `files[]`.
...
- `cmd_pull` calls `git_ops.sync_main`, using the repository OS lock described in §2.3. The legacy `git_ops.do_pull` helper is not this CLI path.
...
- The core holds the repository OS lock around add, commit, and optional main sync. With `--push`, it requires `main` before staging; without `--push`, a local commit can run on the current branch under the same lock.
...
- With `--push`, the scoped local commit is created first, then the private locked sync helper performs the fetch/rebase/push flow in §3.4.1 without reacquiring the lock. Success returns `ok=True, committed=True, pushed=True`; sync failure preserves the local commit and returns `ok=True, committed=True, pushed=False` with sanitized detail and `last-sync-error`.

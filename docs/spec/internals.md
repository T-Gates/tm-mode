...
│   ├── session-start.py          #   SessionStart additionalContext 주입
│   ├── session-log-remind.py     #   UserPromptSubmit 리마인더 + auto_pull 호출
│   ├── auto_pull.py              #   manifest 엔트리 아님. session-log-remind helper
...
1. 설계 목표는 공통 스크립트와 스킬에 에이전트 고유 표기를 두지 않는 것이다. 단 현 reference 공통 훅 일부는 예외적으로 Claude 출력 스키마(`hookSpecificOutput`/`permissionDecision`)와 Codex의 `PreToolUse` 한계를 직접 알고 있다(`session-start.py`, `confirm-action.py`, `session-log-remind.py`; 부록 A.3).
...
- `args`는 문자열 그대로 커맨드 끝에 붙는다. 리스트 파싱이나 shell escaping 재해석은 하지 않는다. 예: manifest의 `"args": "acme-linear-create-allow"`는 `confirm-action.py`의 첫 positional 인자로 들어간다.
...
- `enforcement` 기본값은 `"advisory"`다. 현 코드에서 이 필드는 Codex `sync()`의 `event is None` 경로에서만 경고 문구를 강화한다. 즉 `enforcement: "block"`이고 Codex가 `PreToolUse`를 지원하지 않으면 `[warn] ... (block 강제 상실) → 비활성`을 출력한다. Claude `sync()`는 `enforcement`를 읽지 않는다.
...
|---|---:|---|---|---|
| `session-start.py` | ✅ `SessionStart` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.acme-active` 부재, engine import/수집 실패면 no-op | 활성 시 Claude additionalContext JSON stdout, 항상 exit 0 의도 |
| `session-log-remind.py` | ✅ `UserPromptSubmit` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.acme-active` 부재면 no-op. 활성 시 auto-pull 후 로그 age/프롬프트 카운터 판정 | 필요 시 additionalContext JSON stdout, 정상 exit 0. 단 counter 파일 write 실패는 현재 main에서 별도 catch하지 않음 |
| `auto_pull.py` | ❌ helper | 함수 호출 | 상태파일 throttle, git pull 실패, 예외를 결과 객체로 흡수 | CLI main 없음. `AutoPullResult` 반환, 예외 전파 금지 의도 |
| `auto-commit.py` | ✅ `PostToolUse` | 정규 JSON stdin | event/action 불일치, `.acme-active` 부재, `git_ops` 부재, files 없음, 예외면 no-op | 정규 `files`만 commit 대상으로 넘기고 push 금지, 항상 exit 0 |
| `confirm-action.py` | ✅ `PreToolUse` | 정규 JSON stdin + 첫 argv marker | event 불일치, `.acme-active` 부재, 대상 MCP 불일치, 사람 allow 신호 있으면 통과 | allow 없으면 deny JSON stdout + stderr, exit 2 |
...
5. 현 reference 값:
   ...
   - Codex: `SessionStart`·`UserPromptSubmit`·`PostToolUse` 지원, `PreToolUse = null`, `actions.file_edit = "apply_patch"`, `mcp_tool_format = "{server}.{tool}"`, config 기본 파일 `~/.codex/config.toml`.
...
- Codex `sync()`는 `PreToolUse`가 이벤트 미지원이라 `confirm-action.py`를 등록하지 않는다. `install-mcp`로 linear alias를 등록해도 PreToolUse 차단은 생기지 않는다.
...
- `on` 실행 순서:
  ...
  - `<team_root>/.acme-active`에 빈 문자열을 UTF-8로 쓴다. 부모 디렉토리 생성은 하지 않는다. 기존 파일은 덮어써진다.
...
- `off` 실행 순서:
  ...
  - `<team_root>/.acme-active`가 존재하면 삭제한다. 없으면 그대로 진행한다.
  - `team.farewell`이 있으면 그 문자열을 출력하고, 없으면 `teammode off — 상태 저장됨`을 출력한다.
  - 정상 완료 시 exit 0.
- 멱등성:
  - `on`은 반복 실행해도 같은 `.acme-active` 빈 파일을 다시 쓰고 adapter sync를 다시 수행한다. 배너 파일이 이미 있으면 재생성하지 않는다. upstream은 매번 fetch를 시도할 수 있다.
...
- INDEX:
  ...
  - 파일이 아니면 빈 문자열이다.
- active 상태:
  - `<team_root>/.acme-active` 존재 여부만 본다.
...
- **공통 훅의 에이전트 지식 누출**: 설계 목표는 에이전트 고유 지식을 `agents/<name>/` 아래로 격리하는 것이나, 현 `session-start.py`/`confirm-action.py`/`session-log-remind.py`는 Claude 출력 스키마와 Codex 한계를 직접 알고 있다.
- **bootstrap verify의 Claude 고정**: wire는 감지 에이전트별 어댑터를 호출하지만 verify는 감지 결과와 무관하게 `teammode.py on`을 실행하고, 엔진 `on`은 항상 Claude 어댑터만 로드한다.

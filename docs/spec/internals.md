...
- **에이전트 설정 경로도 명시 only(필수)**: `~/.claude/settings.json`, `~/.codex/config.toml` 등 실 사용자 설정은 **agent별 명시 경로(Claude `--settings`, Codex `--config`) 또는 명시적 설치 플래그(`--install`/`--yes`)** 없이는 건드리지 않는다(§3.1·§4.5).
...
│   └── confirm-action.py         #   PreToolUse/mcp__linear__create_issue 확인 차단
├── skills/
...
1. 설계 목표는 공통 스크립트와 스킬에 에이전트 고유 표기를 두지 않는 것이다. 단 현 reference 공통 훅 일부는 예외적으로 Claude 출력 스키마(`hookSpecificOutput`/`permissionDecision`)를 직접 알고 있다(`session-start.py`, `confirm-action.py`; 부록 A.3). `session-log-remind.py`는 2026-06-21 재설계에서 **평문 stdout 출력**으로 전환해 에이전트 고유 JSON 스키마 의존을 제거했다 — normalize가 그대로 재방출해 Claude가 수신하는 방식.
...
- `enforcement` 기본값은 `"advisory"`다. 현 코드에서 이 필드는 Codex `sync()`의 `event is None` 경로에서만 경고 문구를 강화한다. 즉 `enforcement: "block"`인 manifest 엔트리의 이벤트가 해당 agent events.json에서 `null`이면 `[warn] ... (block 강제 상실) → 비활성`을 출력한다. 현재 Codex는 `PreToolUse`를 지원하므로 confirm/kb guard 차단 훅은 이 fallback 경로를 타지 않는다. Claude `sync()`는 `enforcement`를 읽지 않는다.
...
| `session-log-remind.py` | ✅ `UserPromptSubmit` | 정규 JSON stdin | event 불일치, JSON 파싱 실패, `.teammode-active` 부재면 no-op. 활성 시 멤버 식별(team.config.json members 단일→직접/복수→TEAMMODE_MEMBER env/폴백→전역 sessions 기준) + 내 파일 mtime 기반 age·카운터 판정. check_reset: 내 파일 mtime 변화 또는 날짜(06시 컷) 바뀜 → count=0 + return(안 보챔). (**pull 안 함** — 2026-06-17 P0 hook hang 수정으로 매 프롬프트 pull 을 SessionStart 1회로 분리) | 필요 시 **평문 stdout print**(JSON 아님 — normalize 재방출로 전파). age≥1800 OR count%5==0 발화, 출력에 count 표시. 정상 exit 0. 상태파일 write 실패는 OSError catch(무해). |
...
// agents/codex/events.json (reference) — PreToolUse 지원, file_edit: "apply_patch",
//   mcp_tool_format: "mcp__{server}__{tool}"
```
...
5. 현 reference 값:
   ...
   - Codex: `SessionStart`·`UserPromptSubmit`·`PreToolUse`·`PostToolUse` 지원, `actions.file_edit = "apply_patch"`, `mcp_tool_format = "mcp__{server}__{tool}"`, config 기본 파일 `~/.codex/config.toml`.
...
adapter.py [global-options] uninstall           # teammode 훅 제거 + teammode 스킬 제거
...
주의: 위 표는 어댑터 `adapter.py` 자체의 CLI 계약이다. `install.py --<agent> ...` 디스패처 게이트도 에이전트별 settings 어휘를 인정한다. Claude는 `--settings`, Codex는 `--config`, 둘 다 실설치 의도는 `--install`이다(§4.2).
...
4. 매처 번역:
   ...
   - `{"mcp": {"server": S, "tool": T}}`: `events.json.mcp_tool_format`에 `server=resolve_server_alias(S)`, `tool=T`를 치환한다. 현 `resolve_server_alias`는 항등이다.
...
현 구현의 `install-mcp`는 실제 MCP 서버 실행 command를 제작하지 않는다. 계약은 **연결된 provider의 정규 서버명 alias가 에이전트 설정에 teammode 관리 항목으로 존재하도록 보장**하는 데 있다. 실행 가능한 MCP 서버 정의 작성은 0.2 범위 밖이며 provider 팩의 `mcp.register_hint`를 placeholder에 담는다.
...
4. alias는 `resolve_server_alias(provider)` 결과다. 현 구현은 항상 provider 문자열 그대로다(정규명=별칭).
...
3. 등록 블록 entry는 다음 TOML placeholder다.
   ```toml
...
5. `PreToolUse`/`PostToolUse`:
   ...
   - `tool_input = raw.get("tool_input")`; 없으면 `{}`. `tool_input`이 dict이고 top-level `raw.input`이 문자열이면 `tool_input.input` fallback으로 보탠다. 이는 Codex session tool-call 기록의 `name="apply_patch"`, `input="*** Begin Patch..."` 형태를 보수적으로 수용하기 위한 것이다.
   ...
   - action이 잡힌 경우 기본은 `tool_input.file_path`가 truthy이면 절대경로 변환·경로탈출 검증 없이 `files = [file_path]`, 아니면 `files = []`.
   - Codex `apply_patch`는 실제 hook stdin의 `tool_input.command` 문자열을 우선 지원하고, 호환 후보로 `tool_input.patch` 또는 `tool_input.input` 문자열도 처리한다. 이 문자열에서 `*** Add File:` / `*** Update File:` / `*** Delete File:` / `*** Move to:` 헤더 경로를 모두 추출해 `files = [path, ...]`로 넘긴다. 경로 수 정책은 normalize가 판단하지 않는다. `kb-write-guard.py`는 기존 단일 파일 정책에 따라 2개 이상을 malformed로 deny하고, `auto-commit.py`는 `PostToolUse`에서 `files` 전체를 `paths=`로 커밋한다.
6. MCP 역파싱:
   - `mcp_tool_format` 템플릿을 regex로 바꿔 `{server}`와 `{tool}`을 capture한다.
   - 현 구현은 alias를 canonical server로 역변환하지 않는다. 0.2의 alias 항등 규칙 때문에 `server`가 그대로 정규 서버명이다.
7. 런타임 자가 필터:
...
- **Codex PreToolUse 지원**: Codex hooks 는 `PreToolUse`를 지원한다. 따라서 `confirm-action.py`, `kb-write-guard.py` 같은 `enforcement: block` 훅도 config.toml hooks 블록에 등록한다. Codex 원어 입력은 `hook_event_name`, `tool_name`, `tool_input`, `prompt` 기반으로 정규 스키마에 매핑한다. `apply_patch`의 파일 경로는 실제 hook stdin의 `tool_input.command` patch 문자열 헤더에서 추출한다.
- 독립 구현은 `agents/` 디렉토리 구조나 Python을 그대로 쓸 필요 없다. 보존 대상은 **선언 포맷(manifest.json·events.json)과 의미**이지 구현 언어·파일 배치가 아니다(§6 C2).
- Codex MCP 등록 한계도 정직하게 표면화한다. `install-mcp`는 `config.toml`에 command 없는 placeholder 블록만 쓰며, 실제 기동 가능한 MCP 서버 정의는 사용자가 보강해야 할 수 있다.
- Codex `sync()`는 MCP 매처를 Codex 공식 hook matcher 형식인 `mcp__{server}__{tool}`로 렌더링한다. `install-mcp`로 provider alias가 보장되면 `confirm-action.py`의 PreToolUse 차단도 등록된다.
...
11. `adapter.py uninstall` CLI는 훅 제거와 스킬 제거를 모두 실행한다. Claude 훅 uninstall은 소유 훅 제거 메시지가 없으면 빈 list를 반환할 수 있고, skills uninstall 메시지는 별도로 출력된다. Codex 훅 uninstall은 block 제거 여부에 따라 `[remove] teammode 훅 블록` 또는 `[ok] 제거할 블록 없음`을 반환한 뒤 skills uninstall을 실행한다.
...
### 3.1 on / off (agent settings 경유 — `--root` + agent별 settings/config 또는 `--install`)

`on/off`만 에이전트 어댑터 `sync()`를 호출하므로 settings/config 경로 게이트가 있다. `log/context/pull/commit/update/issue`는 settings를 요구하지 않고, `--settings`/`--config`가 와도 무시한다.

- settings 해석:
  - `--agent <claude|codex>`, `--claude`, `--codex` 명시는 감지 여부보다 우선해 해당 agent만 대상으로 삼는다.
  - `--config <경로>` 단독은 Codex 단일 대상으로 해석한다.
  - `--settings <파일>` 단독은 기존 Claude 격리 동작으로 유지한다.
  - `--agent codex --settings <DIR>`처럼 Codex가 `--settings`를 받을 때는 `<DIR>/codex/config.toml` 격리 config로 해석한다. `<경로>.toml`이면 그 파일을 그대로 쓴다.
  - `--install`이면 홈에서 `.claude`/`.codex`를 감지해 둘 다 토글한다. 감지 agent가 없으면 Claude 실설치 경로로 폴백한다.
  - 위 규칙으로 target을 만들 수 없으면 `[error] --settings <경로> 또는 --config <경로> 또는 --install 이 필요합니다.`를 stderr에 쓰고 exit 2.
- adapter 생성:
  - 대상 agent별 `infra/agents/<agent>/adapter.py`를 `runpy.run_path(..., run_name="__teammode_engine__")`로 로드한다.
  - `Adapter(agent_dir=<infra>/agents/<agent>, manifest_path=<infra>/hooks/manifest.json, settings_path=<resolved>, team_root=<INFRA.parent>)`를 만든다.
...
- `on` 실행 순서:
  ...
  - target agent마다 adapter `sync(mode="on")`를 호출한다.
...
- `off` 실행 순서:
  - target agent마다 adapter `sync(mode="off")`를 호출한다.
  - `<team_root>/.teammode-active`가 존재하면 삭제한다. 없으면 그대로 진행한다.
  - `team.farewell`이 있으면 그 문자열을 출력하고, 없으면 `teammode off — 상태 저장됨`을 출력한다.
  - 정상 완료 시 exit 0.
- 멱등성:
  - `on`은 반복 실행해도 같은 `.teammode-active` 빈 파일을 다시 쓰고 target agent adapter sync를 다시 수행한다. 배너 파일이 이미 있으면 재생성하지 않는다. upstream은 매번 fetch를 시도할 수 있다.
  - `off`는 마커가 없어도 성공 경로로 진행한다. target agent adapter sync는 매번 수행한다.
...
**범위 한정**: 호환 선언은 특정 spec_version에 대한 선언("teammode 호환 (spec 0.2)"). 버전 없는 선언은 무효. 부분 구현은 "partial" 표기로 등재 가능(예: memory-only는 K1~K2·K8 + 골든 중 컨텍스트 조회·세션로그 작성). 부분집합 적정성은 maintainer 리뷰 승인.
...
배지: `![teammode compatible](…/teammode-compatible%20(spec%200.2)-blue)`. **spec_version 반드시 포함**(버전 없는 선언 무효). `partial`/`stale`은 compatible 배지 불가(상태 배지는 가능). 명예 기반 — 허위 확인 시 제거·공지.
...
`team.config.json`의 `services`는 **역할 → provider** 선언이다. 역할명은 도구 중립 어휘로 고정한다: `issues`·`chat`·`docs`·`calendar`. 구현상 `services`는 `None` 또는 `{}`이면 전부 빈 슬롯으로 유효하다. 부분 채움도 유효하다.
...
- **훅 매처**: 0.2 구현은 `providers/<name>.json.action_map`을 컴파일 소비하지 않는다. MCP 매처는 §2.5의 정규 서버명 방식이며, 정규 서버명은 provider 이름과 항등이다.
...
- **공통 훅의 에이전트 지식 누출**: 설계 목표는 에이전트 고유 지식을 `agents/<name>/` 아래로 격리하는 것이나, 현 `session-start.py`/`confirm-action.py`는 Claude 출력 스키마 일부를 직접 알고 있다. `session-log-remind.py`는 2026-06-21 재설계에서 평문 stdout 출력으로 전환해 이 갭을 해소했다.
...
- **Codex 실 훅 입력 스키마 운영 확인됨**: 2026-06-21 격리 `codex exec` 캡처에서 `PreToolUse` stdin은 `hook_event_name="PreToolUse"`, `tool_name="apply_patch"`, `tool_input.command="*** Begin Patch..."` 형태였다. normalize는 이 형태와 세션 tool-call 기록의 `name/input` fallback을 함께 지원한다.
- **install.py uninstall의 Claude MCP 회수 누락**: install wire는 Claude `install-mcp`로 별도 `.claude.json` MCP 등록 파일을 쓸 수 있지만, uninstall 경로는 아직 Claude MCP 등록 파일의 `mcpServers` 엔트리를 역정리하지 않는다. Codex는 단일 `config.toml` 안의 hook/MCP 블록과 skills 제거를 수행한다.
- **`--settings` 의미 불일치**: bootstrap은 `--settings`를 격리 디렉토리로 해석하지만 uninstall은 같은 값을 settings 파일 경로로 그대로 사용한다.
- **normalize 경로 검증 부재**: `tool_input.file_path`와 Codex `apply_patch`에서 추출한 경로는 절대경로화/경로탈출 검증 없이 `files`로 복사되고, `<script>`도 파일명·경로탈출 검증 없이 `HOOKS_DIR / script`로 실행된다.
...
- [x] Codex 훅 입력 JSON 실스키마 운영 확인(2026-06-21 격리 `codex exec` 캡처: `tool_input.command`).

...
**팀 확장 (자유)**: `memory/` 아래 자유 폴더 추가 가능(예: `product/`, `extras/`). 규칙 두 가지 — ① 기존 폴더로 충분하면 새 폴더 금지(증식 방지, 권장), ② 새 폴더는 INDEX.md에 등재(필수). 등재/해제는 reference 동사 `teammode.py memory route {upsert|remove}`(`--root --path --desc --author`)가 담당하며, `memory write`는 미등재 최상위 폴더 감지 시 이 동사를 안내하는 `[hint]` 한 줄을 stdout에 출력한다(자동 등재 아님 — 설명 한 줄은 사람이 확정).

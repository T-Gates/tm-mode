...
tm-mode SPEC v0.3 — 스킬 계층(base/core) + tgates 이식 로드맵 + 온보딩 스킬 명세(§5)
...
## 이식 로드맵 (tgates-toolkit → tm-mode)

tgates-toolkit의 검증된 스킬을 범용 tm-mode로 이식한다. tgates 특정 의존(`TGATES_HOME`·하드코딩 채널/DB ID)은 tm-mode 범용(`--root` 명시·`team.config.json` services 슬롯)으로 번역한다.
...
| 스킬 | 이유 |
|---|---|
| `tgates-onboard` · `credentials` | tm-mode에 `tm-onboard`·금고 이미 있음 (중복) |
...
**정체**: 연결된 docs 슬롯(노션 등)의 페이지를 긁어 주제별로 정리해 팀 `memory/`로 저장하는 core 스킬. 콜드스타트 시드 + 재실행 추가("이 노션 페이지 추가해") 겸용. 판단은 스킬이, 저장은 엔진 `memory write`(§3.6)가 담당한다.
...
**흐름 요약**: ① docs 슬롯 확인 → ② 페이지 목록/트리만 파악(본문 X, 기본 상한 20페이지·깊이 2) → ③ **preview 확인 게이트**(원본→저장 위치·weight 제안·근거 표 + "최대 N개 파일, 파일당 commit/push 1회" 명시 — 이 단일 확인이 weight 일괄 승인을 겸한다) → ④ 페이지별 fan-out 서브에이전트(본문은 서브만 읽음, 파일 직접 쓰기 금지) → ⑤ 메인이 주제별 병합(~10파일 이내, 페이지:파일 1:1 금지) 후 `memory write` 호출 → ⑥ 완료 보고(생성/수정·INDEX·push 상태 구분).

**규칙**:
- weight 기본 📎, 명백한 결정/규칙만 📌 제안, **🔥 자동 제안 금지**. weight 규약("추측 금지")은 preview 표 승인으로 충족한다.
- 본문 하단 `## 출처` 절 필수(외부유래 표시). 파일명은 소스에서 결정적으로 — 재실행 = 같은 파일 replace(멱등).
- 새 최상위 폴더는 `memory route upsert` 등재 선행(§3.6 동적 허용 규칙).

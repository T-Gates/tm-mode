...
**정체**: 연결된 docs 슬롯의 문서 서비스에서 페이지를 긁어 주제별로 정리해 팀 `memory/`로 저장하는 core 스킬. 역할 어휘 원칙 — 실제 서비스명은 `services.docs.provider` 가 답하며, 안내·출처 표기에 그 이름을 읽어 쓴다(제품명 하드코딩 금지, §7.3). 콜드스타트 시드 + 재실행 추가("이 노션 페이지 추가해") 겸용. 판단은 스킬이, 저장은 엔진 `memory write`(§3.6)가 담당한다.

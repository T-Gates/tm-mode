...
- `.tgates-active` 마커를 삭제해 팀모드를 off로 만든다.
...
현재 `infra/skills/base/tm-reset/SKILL.md`의 "먼저 확인" 목록은 `.tgates-active`·settings hook·env·Obsidian·memory 보존만 고지하고, MCP 등록과 skills 설치 흔적이 제거되지 않는다는 사실은 빠뜨린다. 실제 uninstall 동작은 아래 설명과 A.3 갭을 따른다.

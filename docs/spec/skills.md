...
tm-mode SPEC v0.3 — 스킬 계층(base/core) + 레거시 이식 로드맵 + 온보딩 스킬 명세(§5)
...
## 이식 로드맵 (레거시 툴킷 → tm-mode)

레거시 툴킷의 검증된 스킬을 범용 tm-mode로 이식한다. 레거시 특정 의존(전용 HOME env·하드코딩 채널/DB ID)은 tm-mode 범용(`--root` 명시·`team.config.json` services 슬롯)으로 번역한다.

### 이번 이식 — L1 4개

| 스킬 | 출처(레거시) | L1 코어 (즉시 동작) | L2 graceful (연결 시 추가) | 선행 |
|---|---|---|---|---|
| **tm** (on/off) | `legacy` | 엔진 `on`/`off` 동사 래퍼 + 맥락 주입 + 세션로그(`log`/`commit`) | — | 경량 sync |
...
| 스킬 | 이유 |
|---|---|
| `legacy-onboard` · `credentials` | tm-mode에 `tm-onboard`·금고 이미 있음 (중복) |
| `3d-modeling` · `acme-browse` | 특정 팀 전용(예시) |

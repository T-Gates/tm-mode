...
> 상태: design (brainstorm 2026-06-18, Jane) / 다음: writing-plans
...
근거: 거버넌스 공통 로직(unlock 플래그·TTL·세션 매칭·`.acme-active` 가드·팀루트 판별·fail-closed)을 이미 보유. Bash action 을 같은 훅이 처리하면 전부 재사용. 별도 훅이면 이 가드들이 중복돼 표류 위험.
...
| `cp`/`mv` 의 **목적지**가 memory/ | `.acme-active` off (일상작업 — 빌드 안전) |
...
- `.acme-active` 없으면(teammode off) 통과 — 빌드/일상 안전.
...
- **allow**: `cat memory/…`(읽기), `python infra/teammode.py knowledge write …`(동사), unlock 플래그 활성, `.acme-active` off.

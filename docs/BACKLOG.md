...
- (출처: tgates→teammode 마이그레이션 도그푸딩 중 발견. 상세 설계는 teammode 본격 작업 시.)
...
- 결과: 기존 비-teammode 연결(tgates-toolkit이 등록한 linear/slack/notion MCP)을 흡수 대상으로 **탐지 못 함**. 흡수 의도 ↔ 탐지 메커니즘 불일치.
...
## 온보딩 발견성 + "왜+다음" 침묵 — teammode 전반 불친절 (2026-06-20 도그푸딩, 은수 핵심지적)
...
## UX 포팅 — tgates엔 있고 teammode가 안 가져온 것들 (2026-06-20 도그푸딩, 은수)
메타: tgates→teammode 마이그레이션이 엔진(L1)만 가져오고 UX/첫경험을 안 가져옴. 재발명 말고 **포팅**.
- **tm on 출력 포팅**: tgates on(tgates SKILL.md:39-65)은 [환영 + 📊팀원별 상태(🔧하는일/⏭다음/🚧막힌것, 세션로그+Linear In Progress) + 📋지난성과 3~5줄 + 📅일정(Google Cal 오늘~+3일) + 멤버 이모지 🌙은수/😛준형/👽Xian]. teammode tm on은 배너+greeting+summary 한줄 → 빈약.
...
- **statusline 포팅**: tgates는 statusline-command.sh:52 `[T-Gates]`(노란 하드코딩) + `.tgates-active` 마커 조건부. teammode: ①statusline-command.sh에 `.teammode-active` 체크 추가(개인 statusline 유지·병합) ②team.config team.name 동적 읽기(bash+python3) ③settings.json TEAMMODE_HOME 배선(미구현) ④마커 우선순위 teammode>tgates. teammode.py 마커는 이미 구현(:43,:1446).
...
## 정체성(팀명·greeting·farewell) 커스텀 축 + team.id (2026-06-21 은수) — 다음
...
- **정공법 (은수 결정)**: `team.config.json`에 **불변 식별자 `team.id`** 도입(install 1회 생성, 절대 불변) → 크리덴셜·MCP wiring을 `team.id` 기준으로 전환 → `team.name`은 순수 표시용(greeting·farewell·배너·statusline) = 자유 변경.
...
- **참고**: 멀티팀 동시소속은 애초 고려사항 아님(은수) — 식별자는 멀티팀 충돌방지가 명목이었으나 우리 용례엔 불필요. 안정 키(id)면 단일팀에서도 개명 안전.
...
- **teammode는 codex statusMessage 안 넣음** — `infra/agents/codex/adapter.py`가 hooks 블록 생성하되 statusMessage 누락. 마이그레이션 시 빠짐. 준현이 teammode 쓰면 codex statusline 비어있음.
- **할 일 (statusline 두 갈래)**:
  - Claude statusLine = 동적 팀명(오늘 우리 환경에 수동 구현, statusline-command.sh) → install 자동주입 제품화 필요(기존 작업4).
  - **Codex statusMessage = teammode codex adapter에 포팅** (tgates 원본 있음). 단 "tgates hook" 고정 → **동적**(팀명/팀모드 ON, team.config에서)으로 개선.

...
>
> **예정** (대부분 [acme-toolkit](https://github.com/acme)에 이미 구현 — 포팅만 하면 됨): 팀이 **원하는 스킬만 골라 쓰는 선택 설치**, 같은 config로 **표준화된 MCP 세트를 팀 전체가 공유**, L2 서비스(이슈·채팅·문서·캘린더) 연동.

---

## 왜 teammode? (장점)

> **스윗스팟은 2~5인 팀.** 각자 AI로 일하지만 맥락 공유는 꼭 필요한 규모 — 정식 협업 도구를 갖추긴 이르고, 안 맞추면 소통 비용이 커지는 딱 그 지점이다. (7인 이상은 0.2가 동작을 보증하지 않는다.)

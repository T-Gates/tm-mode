...
Each session, the agent automatically *reads* the team context, *records* what it did, and publishes that session log to the team remote.
...
> **Without `pip` (curl):** `curl -fsSL https://raw.githubusercontent.com/T-Gates/tm-mode/refs/tags/v0.1.7/install.sh | sh -s -- join <url>` (`init` too).
...
| 🔒 **Safety first** | Tokens stay in a local vault, real config writes are gated behind `--yes`, and automatic session-log publication is path-scoped, bounded, and recoverable. |
...
Spec: [docs/spec/](docs/spec/README.md) — the single authoritative SPEC v0.4 (English). Contributor map: the Architecture section above (Korean original in the [한국어 section](#tm-mode--한국어)).
...
에이전트가 세션마다 팀 맥락을 자동으로 *읽고*, 한 일을 자동으로 *남긴 뒤* 팀 원격에도 게시한다.
...
> **`pip` 없이 (curl):** `curl -fsSL https://raw.githubusercontent.com/T-Gates/tm-mode/refs/tags/v0.1.7/install.sh | sh -s -- join <url>` (`init` 도 동일).
...
| 🔒 **안전 우선** | 토큰은 로컬 금고, 실 설정 쓰기는 `--yes` 게이트, 세션로그 자동 발행은 경로·시간·복구 범위를 제한. |
...
스펙: [docs/spec/](docs/spec/README.md) — 단일 권위 SPEC v0.4.

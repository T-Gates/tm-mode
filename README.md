...
> **스윗스팟은 2~5인 팀.** 각자 (서로 다른) AI로 일하지만 맥락 공유는 꼭 필요한 규모 — 정식 협업 도구를 갖추긴 이르고, 안 맞추면 소통 비용이 커지는 딱 그 지점이다. (7인 이상은 0.2가 동작을 보증하지 않는다.)
...
| 🤖 **크로스에이전트** | 팀원마다 **다른 에이전트를 써도**(누구는 Claude Code, 누구는 Codex) 같은 팀 메모리를 공유. 도구 통일 강제 없고, 한 사람이 에이전트를 갈아타도 맥락은 그대로 — 에이전트 종속 0. |
...
| 🧩 **에이전트별 재정의 불필요** | 스킬을 `infra/skills/base/`에 한 번 두면 Claude·Codex 양쪽 스킬 폴더에 자동 배포 — 에이전트마다 따로 만들 필요 없다. |
| 🎚️ **스킬 관리** | 팀이 쓸 스킬을 한 곳에서 정의·공유하고, 원하는 스킬만 골라 설치한다. |
| 🔏 **로그 프라이버시** | 세션로그엔 **팀 작업만** 기록하도록 안내된다(개인 내용 제외). 기록되는 시점이 매 세션 명시돼, "나도 모르게 남는" 일이 없다. |
...
> - **에이전트가 섞인 팀** → "한 분은 Claude Code, 한 분은 Codex 쓰셔도 돼요. 도구 통일할 필요 없이 같은 팀 메모리를 공유합니다." (전부 같은 에이전트일 때만 쓰는 게 아니라, **섞여 있어도** 된다는 점을 분명히 하라 — 도입 조건으로 오해받기 쉬운 지점이다.)

...
**시작은 한 마디.** 에이전트에게 *"이 레포 셋업해줘"* — 설정 파일도, 문서도 먼저 읽을 필요 없다. 진입장벽이 거의 0이다.

**그리고 쓸수록 강해진다 (복리).** 세션로그는 사라지지 않고 누적된다 — 일할수록 자동주입되는 맥락이 두꺼워지고, 나중에 합류하는 팀원도 첫날부터 팀의 전체 히스토리를 안고 시작한다. 처음엔 비어 있어 가볍지만, 팀의 모든 세션이 복리로 쌓여 갈수록 "소통 비용 없이 아는" 효과가 커진다.

**기록은 당신에게도 남는다.** 결정의 이유도, 막혔던 지점도, 그날 한 일도 — 나중에 언제든 다시 꺼내 본다. 분기 회고, 이력서·자기소개서, 블로그 글감까지: "내가 언제 무엇을 왜 했는지"가 이미 다 적혀 있다.

...
> **Without `pip` (curl):** `curl -fsSL https://raw.githubusercontent.com/T-Gates/tm-mode/refs/tags/v0.1.5/install.sh | sh -s -- join <url>` (`init` too).
...
recording
  └ agent appends the session log → auto-commit hook: scoped local commit
    + bounded foreground push (non-ff: fetch → safe rebase --autostash → re-push)
...
> **`pip` 없이 (curl):** `curl -fsSL https://raw.githubusercontent.com/T-Gates/tm-mode/refs/tags/v0.1.5/install.sh | sh -s -- join <url>` (`init` 도 동일).
...
세션 중 기록
  └ 에이전트가 세션로그 이어쓰기 → auto-commit 훅: 경로 한정 로컬 커밋
    + 제한시간 내 전경 push(non-ff면 fetch → 안전 확인 후 rebase --autostash → 재push)
